...
async function main() {
  ...
  await Promise.all(monoStackProblems.map((data) => prisma.problem.create({ data })));
  ...
  await Promise.all(twoPointersProblems.map((data) => prisma.problem.create({ data })));
